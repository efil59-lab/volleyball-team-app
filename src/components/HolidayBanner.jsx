 ...
 * מסך בלי תנאי מסביב.
 *
 * hide: כשיש יום הולדת. הברכה האישית גוברת על החג, ושניהם יחד על אותו מסך
 * זה עומס. במקרה כזה מוצגת שורה דקה (slim) במקום הכרטיס המלא.
 */
export default function HolidayBanner({ slim = false, hide = false }) {
  if (hide) return null;
  const h = holidayFor();
  if (!h) return null;
